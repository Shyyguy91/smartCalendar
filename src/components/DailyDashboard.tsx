...
export default function DailyDashboard() {
  ...
  if (activeTab === "Curriculum Tracker") {
    return <main className="min-h-screen bg-slate-50 px-4 py-6 text-slate-900"><div className="mx-auto max-w-6xl"><button type="button" onClick={() => setActiveTab("Today")} className="mb-6 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-bold">Back to Today</button><CurriculumTracker tracks={curriculum} onChange={setCurriculum} onSync={syncCurriculum} onImportWeeklyPlan={() => setScheduleImportOpen(true)} /><ScheduleImportModal isOpen={scheduleImportOpen} selectedDate={dateKey} onClose={() => setScheduleImportOpen(false)} onImport={importSchedulePayloads} /></div></main>;
  }
